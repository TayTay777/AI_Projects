...
With all of these foundational classes working for our searches it made the execution for Breadth-First 
and Depth-First searches much more simple. The design of Depth-First search was to make a stack of 
carts. Each cart as described before contains x number of sacks along with all of the unpackedItems. 
To exercise a solution The first cart is popped off of the stack and manipulated. First the code
checks to see if the current popped cart is a solution. If it is a solution, the DFS is done, and 
prints the solution. If the popped cart is not a solution then copies of the cart are made equaling 
the amount of the number of sacks i.e. if there is 4 sacks, then 4 cart copies are made. This is to
make sure the next item is put into every bag number. Then these copies are added to the stack, and 
the popped cart is discarded. The process keeps repeating until the stack is empty.
...
the peeked cart is a solution, if it is not it is copied and manipulated like the DFS. If it is a 
solution, the solution is stored and not other copies are made. Once done we then add the new manipulated 
copies of cart objects into the queue and remove the cart being peaked at.
